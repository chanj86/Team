...
class AGENT:
	...
	def __init__(self,id):
		...
		#행위자 지식 계층
		self.mu = np.random.randint(10,40)
		self.sigma = np.random.uniform(2,5)
...
def knowledge_iteraction(AGENT, agent_list):
	...
	for a in agent_list:
		...
		for n in a.neighbor:
			...
			if a.util < mate.util and (a.rank[0]/a.rank[1]) > 0.5 and np.random.rand() < 0.5 : #상대방의 효용이 더 높을 때 그리고 하위 50%의 성과를 지닐때
...
def knowledge_iteraction_delay(AGENT, agent_list):
	...
	for a in agent_list:
		...
		for n in a.neighbor:
			...
			if a.util < mate.util and (a.rank[0]/a.rank[1]) > 0.5 and np.random.rand() < 0.3 : #상대방의 효용이 더 높을 때 그리고 하위 50%의 성과를 지닐때
...
#결과데이터
#perf_df = pd.DataFrame([],index = list(range(0,EXP_ITER)), columns=list(range(0,iter)))
perf_df = pd.DataFrame([], columns = list(range(0,EXP_ITER)), index = list(range(0,iter+1)))
util_df = pd.DataFrame([], columns = list(range(0,EXP_ITER)), index = list(range(0,iter+1)))
bias_df = pd.DataFrame([], columns = list(range(0,EXP_ITER)), index = list(range(0,iter+1)))
...
for ITER in range(0,EXP_ITER):
	...
	print('initial knowledge bias =', 1/knowledge_bias(AGENT, agent_list))
	...
	## Step3) 데이터 업데이트 df[column_name][raw_name]
	# 조직의 퍼포먼스 데이터 업데이트
		perf_df[ITER][i] = AGENT.perf_org
		util_df[ITER][i] = average_util(AGENT, agent_list)
		bias_df[ITER][i] = 1/knowledge_bias(AGENT, agent_list)
...
perf_df.to_csv('result\\perf_df_{}.csv'.format(time_str), header=True)
util_df.to_csv('result\\util_df_{}.csv'.format(time_str), header=True)
bias_df.to_csv('result\\bias_df_{}.csv'.format(time_str), header=True)
